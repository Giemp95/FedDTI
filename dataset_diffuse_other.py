...
dataset_train = pd.read_csv(data_dir + "/kiba_train_split2.csv", sep=',', header=0)
dataset_test = pd.read_csv(data_dir + "/kiba_test_split2.csv", sep=',', header=0)
...
for var in [0.0001, 0.1, 0.2, 0.4, 0.7, 1, 2, 10, 100]:
    ...
    if not TEST:
        run_path = data_dir + "/run_" + str(clients*2) + "_" + str(var)
    ...
    for client in range(clients):
        ...
        if not TEST:
            client_path = run_path + "/client_" + str(client+clients)
